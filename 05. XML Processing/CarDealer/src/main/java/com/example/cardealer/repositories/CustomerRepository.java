...
import com.example.cardealer.models.dtos.exports.CustomerTotalSalesViewDto;
...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("SELECT new com.example.cardealer.models.dtos.exports.CustomerTotalSalesViewDto(c.name, COUNT(s), SUM(p.price)) " +
            "FROM Sale s JOIN s.customer c JOIN s.car car JOIN car.parts p " +
            "GROUP BY c.id " +
            "HAVING COUNT(s) > 0 " +
            "ORDER BY SUM (p.price) DESC , COUNT(s) DESC ")
    List<CustomerTotalSalesViewDto> getAllCustomerSales();
