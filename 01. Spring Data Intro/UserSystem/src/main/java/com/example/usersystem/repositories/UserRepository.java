package com.example.usersystem.repositories;

import com.example.usersystem.models.User;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.stereotype.Repository;

import java.time.LocalDateTime;
import java.util.List;

@Repository
public interface UserRepository extends JpaRepository<User, Long> {
    List<User> findAllByEmailEndingWith(String emailProvider);

    boolean existsByEmailEndingWith(String emailProvider);

    List<User> findAllByLastTimeLoggedInIsBefore(LocalDateTime lastTimeLoggedIn);

    int countAllByIsDeletedIsTrue();

    @Modifying
    int deleteAllByIsDeletedTrue();

    List<User> findAllByIsDeletedTrue();
}
